...
(function () {
  ...
  //  scaleTime: convert MEI time to Audio-time (in milliseconds)
  //  unscaleTime: audio-time to MEI time
  ...
  // MUSIC OBJECT
  // Basically a Verovio wrapper.
  // Since Verovio only allows one instance at a time, this means that
  //  in order to make multiple music components playable (each playable
  //  component requires a separate Verovio state) we need the
  //  single Verovio instance to switch states (i.e. MEI data) on the fly
  ...
  class Music {
    ...
    constructor() {
      ...
      this.meiRegistry = {};
      this.currentMeiFileUrl = undefined;
    ...
    updateTo(meiFileUrl) {
      ...
      if (useCurrentMei) {
        ...
        returnVerovio = this.loadMeiFile(verovioToolkit, meiFileUrl, meiRegistry);
      }

      return returnVerovio;
    }

    // Load the MEI from a file and load it into verovio

    loadMeiFile(verovioToolkit, meiFileUrl, meiRegistry) {
      meiRegistry[meiFileUrl] = await fetch(meiFileUrl);
      this.updateTo(meiFileUrl);
      return verovioToolkit;
    }

    /*
    // Change the MEI state of Verovio (if not already in that state)

    setMeiState(verovioToolkit, meiFileUrl) {

      if (this.currentMeiFileUrl !== meiFileUrl) {
        let newMei = this.meiRegistry[meiFileUrl];
        verovioToolkit.loadData(newMei);
        this.currentMeiFileUrl = meiFileUrl;
      }
    ...
    */
    ...
    getSVG() {
      // this.verovioToolkit.getSvg
    }

    getTimeForElement(id, meiFileUrl) {
      this.updateTo(meiFileUrl);
      return this.verovioToolkit.getTimeForElement(id);
    }

    getElementsAtTime(time, meiFileURL) {
      this.updateTo(meiFileURL);
      return // FINISH
    }

...
/* VARIABLES */
	...
	var waypointEpigram = '._epigram--english';
	var waypointEpigramGerman = '._epigram--german';
...
//http://jennamolby.com/how-to-display-dynamic-content-on-a-page-using-url-parameters/
	...
	*/
	...
	var waypoint = $(waypointMotto).waypoint({
		...
		handler: function(direction) { // triggered when the top of the element hits the top of the viewport
			if(direction === 'down') { // if scrolling down the page, change zooming page to 2/4
				...
				if($(waypointMotto).parent().hasClass('is-shown')) { // english and latin motto page
					zoomingViewer.goToPage(myEmblemDataNum * 4);
				}
				else {
					zoomingViewer.goToPage(myEmblemDataNum * 4 - 1); // german motto page
				}
			}
			else { // if scrolling back up the page
	...
	// /*** IMAGE WAYPOINT ***/
	// // instantiate the global Waypoint class and pass an options object to it. the two paramaters required are element and handler
	var waypoint = $(waypointImage).waypoint({
		...
		handler: function(direction) { // triggered when the top of the element hits the top of the viewport
			if(direction === 'down') { // if scrolling down the page, change zooming page to 2/4
				...
				if($(waypointMotto).parent().hasClass('is-hidden')) {
					zoomingViewer.goToPage(myEmblemDataNum * 4 - 1);
				}
				else { // latin motto page
					zoomingViewer.goToPage(myEmblemDataNum * 4);
				}
			}
			else if (direction === 'up') { // if scrolling back up the page
				console.log("hit image waypoint up");
				if($(waypointMotto).parent().hasClass('is-hidden')) { // english and latin motto page
					zoomingViewer.goToPage(myEmblemDataNum * 4 - 1);
					console.log("image up not shown");
					console.log($(waypointMotto));
				}
				else { // german motto page
					zoomingViewer.goToPage(myEmblemDataNum * 4);
					console.log("image up shown");
					console.log($(waypointMotto).parent());
				}
			}
			else {
	...
	var waypoint = $(waypointEpigram).waypoint({
		...
		handler: function(direction) { // triggered when the top of the element hits the top of the viewport
			if(direction === 'down') { // if scrolling down the page, change zooming page to 2/4 if Latin/English is active or 1/4 if German is active
				...
				if($(waypointEpigram).parent().hasClass('is-shown')) {
					zoomingViewer.goToPage(myEmblemDataNum * 4);
					console.log("shown");
					console.log($(waypointEpigram));
				}
				else {
					zoomingViewer.goToPage(myEmblemDataNum * 4 - 1);
	...
	function changeEnglishWaypoints() {
		englishEpigramWaypoint.enable();
		germanEpigramWaypoint.disable();
	}

	function changeGermanWaypoints() {
		// germanEpigramWaypoint.enable();
		englishEpigramWaypoint.disable();
	}
	...
	germanEpigramWaypoint = new Waypoint({
		...
		//context: document.querySelector('.original.is-shown'),
		offset: 350 // moving the trigger location from 0 at the top of the viewport
	})
